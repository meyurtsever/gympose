"""
Convert RTMPose 20-class dataset to 6-class dataset.

This script applies the same 20->6 mapping used for VIT dataset
...
# Load the 20-class RTMPose dataset
print("Loading RTMPose 20-class dataset...")
df = pd.read_csv('yoga_poses_dataset_rtmpose_20class.csv')
print(f"Loaded {len(df)} samples with {df['class_name'].nunique()} classes")

# Load the 20->6 class mapping
print("\nLoading 20->6 class mapping...")
...
# Display statistics
print("\n" + "="*60)
print("RTMPOSE 6-CLASS DATASET STATISTICS")
print("="*60)
...
output_path = 'yoga_poses_dataset_rtmpose_6class.csv'
df.to_csv(output_path, index=False)
print(f"\n✓ Saved 6-class RTMPose dataset to: {output_path}")
print("\n" + "="*60)
