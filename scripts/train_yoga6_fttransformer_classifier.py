"""
Phase 3: FT-Transformer Average Performance Evaluation (6-Class Dataset)

This script trains the FT-Transformer model 5 times with different random seeds
to compute average performance and standard deviation on the 6-class dataset.
...
- Saves results to fttransformer_6class_5run_results/ folder
- Creates text files with summary statistics and model architecture info
- Saves best performing model for each run
"""
...
class FeatureTokenizer(nn.Module):
    """Converts each numerical feature into a learnable embedding token."""
    def __init__(self, num_features, embedding_dim, dropout=0.1):
        super(FeatureTokenizer, self).__init__()
        
        # Each feature gets its own linear projection
    ...
    def forward(self, x):
        ...
        Returns:
            tokens: (batch_size, num_features + 1, embedding_dim)
        """
        batch_size = x.size(0)
        ...
        for i, embed_layer in enumerate(self.feature_embeddings):
            # (batch_size, 1) -> (batch_size, embedding_dim)
            feature_token = embed_layer(x[:, i:i+1])
            tokens.append(feature_token.unsqueeze(1))
        
        # Stack all feature tokens: (batch_size, num_features, embedding_dim)
...
class FTTransformerClassifier(nn.Module):
    ...
    def __init__(self, num_features, num_classes, 
                 embedding_dim=128, num_layers=6, num_heads=8, 
                 ff_dim=512, dropout=0.1):
        super(FTTransformerClassifier, self).__init__()
        
        # Feature tokenization
        self.tokenizer = FeatureTokenizer(num_features, embedding_dim, dropout)
    ...
    def forward(self, x):
        ...
        """
        # Tokenize features: (batch_size, num_features + 1, embedding_dim)
...
class YogaPoseFTTransformerClassifier:
    """FT-Transformer based yoga pose classifier."""
    ...
    def train_model(self, train_loader, val_loader, num_classes, num_features, 
                   epochs=100, lr=0.001, patience=20, warmup_epochs=10, verbose=False):
...
def run_single_experiment(run_id, data_path, device, verbose=False):
    ...
    classifier = YogaPoseFTTransformerClassifier(random_state=random_state, device=device)
...
def save_results(all_metrics, metrics_summary, save_dir):
    ...
    with open(save_dir / 'fttransformer_individual_runs.json', 'w') as f:
    ...
    with open(save_dir / 'fttransformer_average_results.json', 'w') as f:
        json.dump(summary_data, f, indent=2)
    
    print(f"\n  Results saved to:")
    print(f"    {save_dir / 'fttransformer_individual_runs.json'}")
    print(f"    {save_dir / 'fttransformer_average_results.json'}")
...
def plot_results(all_metrics, metrics_summary, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(save_dir / 'fttransformer_runs_comparison.png', dpi=300, bbox_inches='tight')
    print(f"    {save_dir / 'fttransformer_runs_comparison.png'}")
    plt.close()

def save_model_summary(results_dir, model, num_features, num_classes):
    """Save model architecture summary to text file."""
    summary_file = results_dir / 'fttransformer_model_summary.txt'
    ...
    with open(summary_file, 'w') as f:
        f.write("=" * 80 + "\n")
        f.write("FT-TRANSFORMER MODEL ARCHITECTURE SUMMARY (6-CLASS)\n")
        f.write("=" * 80 + "\n\n")
        
        f.write("Model Type: FT-Transformer (Feature Tokenizer + Transformer)\n")
        ...
        f.write("-" * 80 + "\n")
        f.write("Feature Tokenizer:\n")
        f.write(f"  - Input Features: {num_features}\n")
        f.write(f"  - Embedding Dimension: 128\n")
        f.write(f"  - Feature Embeddings: {num_features} x Linear(1 -> 128)\n")
        f.write(f"  - [CLS] Token: Learnable (1, 1, 128)\n")
        f.write(f"  - Total Tokens: {num_features + 1} (features + [CLS])\n")
        f.write("  - LayerNorm + Dropout(0.1)\n\n")
        
        f.write("Transformer Encoder:\n")
        f.write("  - Number of Layers: 6\n")
        f.write("  - Number of Heads: 8\n")
        f.write("  - Embedding Dimension: 128\n")
        f.write("  - Feed-Forward Dimension: 512\n")
        f.write("  - Dropout: 0.1\n")
        f.write("  - Attention: Multi-Head Self-Attention\n")
        f.write("  - Normalization: LayerNorm (pre-norm)\n\n")
        
        f.write("Classification Head:\n")
        f.write("  - Input: [CLS] token embeddings (128)\n")
        f.write("  - LayerNorm(128)\n")
        f.write("  - Linear: 128 -> 512\n")
        f.write("  - GELU activation\n")
        f.write("  - Dropout(0.1)\n")
        f.write(f"  - Linear: 512 -> {num_classes}\n\n")
        ...
        f.write("-" * 80 + "\n")
        f.write("MODEL ADVANTAGES\n")
        f.write("-" * 80 + "\n")
        f.write("- Designed specifically for tabular data\n")
        f.write("- Feature tokenization preserves individual feature semantics\n")
        f.write("- Attention mechanism learns feature interactions\n")
        f.write("- More parameter-efficient than CNNs for this task\n")
        f.write("- [CLS] token aggregates global information for classification\n\n")
...
def save_summary_text(summary_text, save_dir):
    ...
    save_dir = Path(save_dir)
    summary_file = save_dir / 'fttransformer_5_run_summary.txt'
...
def main():
    ...
    print("\n" + "=" * 80)
    print("FT-TRANSFORMER AVERAGE PERFORMANCE EVALUATION (6-CLASS)")
    print("=" * 80)
    ...
    # Configuration
    #data_path = "output_dataset/yoga_poses_dataset_vit_6class.csv"
    data_path = "output_TCF_gymnastics_yolo11x_640px_04threshold/tcf_poses_dataset_vit_6class.csv"
    results_dir = Path("results/fttransformer_6class_5run_results")
    ...
    sample_model = FTTransformerClassifier(
        num_features=num_features,
        num_classes=num_classes,
        embedding_dim=128,
        num_layers=6,
