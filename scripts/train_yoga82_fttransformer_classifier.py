"""
FT-Transformer Average Performance Evaluation (Unified Script for 6/20/82-Class)
...
Usage:
    python average_train_fttransformer_classifier.py --data output_dataset/yoga_poses_dataset_vit_6class.csv
    python average_train_fttransformer_classifier.py --data output_dataset/yoga_poses_dataset_vit_20class.csv
    python average_train_fttransformer_classifier.py --data output_dataset/yoga_poses_dataset_vit_82vanilla.csv
"""
...
class YogaPoseFTTransformerClassifier:
    ...
    def train_model(self, train_loader, val_loader, num_classes, num_features, 
                   epochs=200, lr=0.001, patience=20, warmup_epochs=10, verbose=False):
...
def main():
    ...
    parser = argparse.ArgumentParser(description='FT-Transformer Yoga Pose Classifier')
    ...
    print("\n" + "=" * 80)
    print(f"FT-TRANSFORMER AVERAGE PERFORMANCE EVALUATION ({dataset_type.upper()})")
    print("=" * 80)
    ...
    # Configuration
    data_path = args.data
    results_dir = Path(f"results/fttransformer_{dataset_type}_5run_results")
