"""
DenseNet121 Average Performance Evaluation (Unified Script for 6/20/82-Class)
...
Usage:
    python average_train_densenet121_classifier.py --data output_dataset/yoga_poses_dataset_vit_6class.csv
    python average_train_densenet121_classifier.py --data output_dataset/yoga_poses_dataset_vit_20class.csv
    python average_train_densenet121_classifier.py --data output_dataset/yoga_poses_dataset_vit_82vanilla.csv
"""
...
class YogaPoseDenseNetClassifier:
    ...
    def train_model(self, train_loader, val_loader, num_classes, input_dim, 
                   epochs=200, lr=0.001, patience=20, warmup_epochs=10, 
    ...
    def evaluate(self, test_loader):
        ...
        top5_acc = top_k_accuracy_score(y_true, y_probs, k=min(5, y_probs.shape[1]))
...
def run_single_experiment(run_id, data_path, device, save_dir, dataset_type, epochs, verbose=False):
...
def save_model_summary(results_dir, model, num_features, num_classes, dataset_type):
    ...
    with open(summary_file, 'w', encoding='utf-8') as f:
        f.write("=" * 80 + "\n")
        f.write(f"DENSENET121 MODEL ARCHITECTURE SUMMARY ({dataset_type.upper()})\n")
        f.write("=" * 80 + "\n\n")
        ...
        f.write("Epochs: 200 (with early stopping, patience=20)\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='DenseNet121 Yoga Pose Classifier')
    ...
    print("\n" + "=" * 80)
    print(f"DENSENET121 AVERAGE PERFORMANCE EVALUATION ({dataset_type.upper()})")
    print("=" * 80)
    ...
    # Configuration
    results_dir = Path(f"results/densenet121_{dataset_type}_5run_results")
    ...
    for run_id in range(5):
        metrics = run_single_experiment(run_id, args.data, device, results_dir, 
                                       dataset_type, args.epochs, verbose=True)
    ...
    save_model_summary(results_dir, sample_model, num_features, num_classes, dataset_type)
